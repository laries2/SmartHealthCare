    ...
    <div class="col-sm-10">
        <h3>Diagnosis validation</h3>
         ...
         <div class="doct">
             ...
             $email = mysqli_real_escape_string($conn,$_POST['diagnosis']);



             // Using MySQL Insert to post the data to the database table courses
             $sql = "update patients set surname = '$surname',last_name = '$lastName',first_name = '$firstName',diagnosis = '$email' where national_id = '$nationalID'";

             if ($conn -> query($sql) == true){
                 print "<p>Validation done Successfully</p>";
                 print "<a href='doctorDashboard.php' class='btn btn-outline-success'>Dashboard</a>";
             }
             else{
                 print "<p>Doctor details not updated</p>";
                 print "<a href='diagnosis.php' class='btn btn-outline-danger'>Try Again</a>";
             }
             mysqli_close($conn);
             ?>
