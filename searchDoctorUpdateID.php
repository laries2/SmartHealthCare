...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-sm-10">
            <h3>Doctor Search by National-ID</h3>

                <div class="doct">
                    <form action="displayDoctorUpdateID.php" method="post">
                        <select name="idSearch" id="" class="form-control" required>
                            <?php
                            $conn = mysqli_connect("localhost","root","","smart");
                            if (!$conn){
                                die("Database Connection Error : " . mysqli_connect_error());
                            }
                            $sql = "select national_id from doctors";
                            $result = $conn -> query($sql);
                            if ($result -> num_rows > 0){
                                // If the table has data
                                while ($row = $result -> fetch_assoc()){
                                    ?>
                                    <option value="<?php echo $row['national_id'];?>"><?php echo $row['national_id'];?></option>
                                    <?php
                                }
                            }
                            mysqli_close($conn);
